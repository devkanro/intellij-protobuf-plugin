...
object BufLintAllowCommentIgnoresFieldSchema : BufFieldSchema(
    ...
    "The `allow_comment_ignores` key is optional, and turns on comment-driven ignores. " +
        "We do not recommend using this option in general, however in some situations it is unavoidable.",
...
object BufLintEnumZeroValueSuffixFieldSchema : BufFieldSchema(
    ...
    "The `enum_zero_value_suffix` key is optional, and controls the behavior of the " +
        "`ENUM_ZERO_VALUE_SUFFIX` lint rule. By default, this rule verifies that the zero value " +
        "of all enums ends in `_UNSPECIFIED`, " +
...
object BufLintRpcAllowSameRequestResponseFieldSchema : BufFieldSchema(
    ...
    "The `rpc_allow_same_request_response` key is optional, " +
        "and allows the same message type to be used for a single RPC's " +
        "request and response type. **We do not recommend using this option in general.**",
    BufSchemaScalarType.BOOL,
    true,
)
...
object BufLintRpcAllowGoogleProtobufEmptyRequestsFieldSchema : BufFieldSchema(
    ...
    "The `rpc_allow_google_protobuf_empty_requests` key is optional, " +
        "and allows RPC requests to be `google.protobuf.Empty` messages. " +
        "This can be set if you want to allow messages to be void forever, " +
        "that is, to never take any parameters. **We do not recommend using this option in general.**",
    BufSchemaScalarType.BOOL,
    true,
)
...
object BufLintRpcAllowGoogleProtobufEmptyResponsesFieldSchema : BufFieldSchema(
    ...
    "The `rpc_allow_google_protobuf_empty_responses` key is optional, " +
        "and allows RPC responses to be `google.protobuf.Empty` messages. " +
        "This can be set if you want to allow messages to never return any parameters. " +
        "**We do not recommend using this option in general.**",
    BufSchemaScalarType.BOOL,
    true,
)
...
object BufLintServiceSuffixFieldSchema : BufFieldSchema(
    ...
    "The `service_suffix` key is optional, and controls the behavior of the `SERVICE_SUFFIX` lint rule. " +
        "By default, this rule verifies that all service names are suffixed with `Service`.",
