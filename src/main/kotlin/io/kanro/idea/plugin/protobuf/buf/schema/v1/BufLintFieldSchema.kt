...
object BufLintIgnoreOnlyFieldSchema : BufFieldSchema(
    "ignore_only",
    "The `ignore_only` key is optional, " +
        "and allows directories or files to be excluded from specific lint rules " +
        "when running buf lint by taking a map from lint rule ID or category to path. " +
        "As with ignore, the paths must be relative to the buf.yaml.",
    BufObjectSchema(
        bufLintRules.map {
...
val bufLintRules =
    listOf(
        BufEnumValueSchema(
            "MINIMAL",
            "The MINIMAL category includes DIRECTORY_SAME_PACKAGE, " +
                "PACKAGE_DEFINED, PACKAGE_DIRECTORY_MATCH, PACKAGE_SAME_DIRECTORY rules.",
        ),
        BufEnumValueSchema(
        ...
        BufEnumValueSchema(
            "BASIC",
            "The BASIC category includes everything from the MINIMAL category, " +
                "and adds basic style rules that are widely accepted as standard Protobuf style. " +
                "These rules should generally be applied for all Protobuf schemas.",
        ),
        ...
        BufEnumValueSchema(
            "IMPORT_NO_WEAK",
            "This rule outlaws declaring imports as weak. " +
                "If you didn't know this was possible, forget what you just learned in this sentence, " +
                "and regardless do not use these.",
        ),
        BufEnumValueSchema(
            "IMPORT_NO_PUBLIC",
            "Similar to the IMPORT_NO_WEAK rule, this rule outlaws declaring imports as public. " +
                "If you didn't know this was possible, forget what you just learned in this sentence, " +
                "and regardless do not use these.",
        ),
        BufEnumValueSchema(
            "DEFAULT",
            "The DEFAULT category includes everything from the BASIC category, " +
                "as well as some other default style rules.",
        ),
        BufEnumValueSchema(
            "ENUM_VALUE_PREFIX",
        ...
        BufEnumValueSchema(
            ...
            "This rule says that all .proto files must be named in lower_snake_case.proto. " +
                "This is the widely accepted standard.",
        ),
        BufEnumValueSchema(
            "RPC_REQUEST_STANDARD_NAME",
            "These rules enforce the message name of RPC request.",
        ),
        BufEnumValueSchema(
            "RPC_RESPONSE_STANDARD_NAME",
            "These rules enforce the message name of RPC responses.",
        ),
        BufEnumValueSchema(
        ...
        BufEnumValueSchema(
            ...
            "This rule enforces that the last component of a package must be a " +
                "version of the form v\\d+, v\\d+test.*, v\\d+(alpha|beta)\\d*, or v\\d+p\\d+(alpha|beta)\\d*, " +
                "where numbers are >=1.",
        ),
