...
object BufBreakingFieldSchema : BufFieldSchema(
    "breaking",
    "The `breaking` key is optional, and specifies the breaking change detection rules " +
        "enforced on the files contained within the module.",
    BufBreakingSchema,
    true,
)
...
object BufBreakingIgnoreFieldSchema : BufFieldSchema(
    "ignore",
    "The `ignore` key is optional, and allows directories or files to be excluded from " +
        "all breaking rules when running buf breaking. The specified directory or file paths " +
...
object BufBreakingIgnoreOnlyFieldSchema : BufFieldSchema(
    "ignore_only",
    "The `ignore_only` key is optional, and allows directories or files to be excluded " +
        "from specific breaking rules when running buf breaking by taking a map from breaking " +
        "rule ID or category to path.",
    BufObjectSchema(
...
object BufBreakingIgnoreUnstablePackagesFieldSchema : BufFieldSchema(
    ...
    "The `ignore_unstable_packages` key is optional, " +
        "and ignores packages with a last component that is one of the unstable forms recognized by " +
        "PACKAGE_VERSION_SUFFIX.",
    BufSchemaScalarType.BOOL,
    true,
)

val bufBreakingRules =
    listOf(
