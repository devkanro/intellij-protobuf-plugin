...
object GrpcRequestConverter : RequestConverter<GrpcRequest>() {
    ...
    override fun psiToCommonRequest(
        ...
        return ApplicationManager.getApplication().runReadAction<GrpcRequest> {
            ...
            val requestBodyGroup = element.requestBody as? HttpRequestMessagesGroup
                ?: throw IllegalStateException("Unsupported body type.")
            val requests = requestBodyGroup.requestMessageList.mapNotNull {
                if (it !is HttpMessageBody) return@mapNotNull null
                it.text
            }

            val rpc = element.requestTarget?.references?.filterIsInstance<GrpcMethodReference>()?.firstOrNull()
            ...
            val inputMessage = rpc.input()?.typeName?.reference?.resolve() as? ProtobufMessageDefinition
            ...
            val outputMessage = rpc.output()?.typeName?.reference?.resolve() as? ProtobufMessageDefinition
                ?: throw IllegalStateException("Unsolvable rpc input '${input.typeName.text}'.")
